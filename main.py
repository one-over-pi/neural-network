...
def score(results):
    ...
    # Divided by number of tests occuring to standardise the result across any number of tests
    return points/len(results)*100


# Mutates the weights and baises
...
# Sigmoid function to give non-linear capabilities to the neural network
def sigmoid(x):
...
def network(nodeStructure, weights, baises, n):
    # Initiate nodes
    ...
    # adding the bais, and running it through the sigmoid function
    for i in range(len(nodeStructure)-1):
        for k in range(nodeStructure[i+1]):
            for j in range(nodeStructure[i]):
                nodes[i+1][k] += nodes[i][j]*weights[i][j]+baises[i][j]
            nodes[i+1][k] = sigmoid(nodes[i+1][k])
            sigmoid(nodes[i+1][k])

    # Only returning first output node
...
def test(nodeStructure, w, b, testDepth):
    ...
    for i in range(testDepth):
        output.append(network(nodeStructure, w, b,
                      [random.random()*4-2 for n in range(nodeStructure[0])]))
    return score(output)


def trainNetwork(nodeStructure):
    # Initiate random weights and baises
    """
    ...
    # Record weights and baises to console
    print("Weights " + str(w) + "\n")
    print("Baises " + str(b) + "\n")
    ...
    while training:
        for g in range(generations):
            # Create variation of weights and baises to comparitively evaluate
            # intensity = min(max(100/bestGeneration[0], 1), 0.0001)
            # High evolution intensity
            intensity = 1
            ...
            for v in range(variations):
                ...
                if (curScore > bestGeneration[0]):
                    ...
                    if (curScore > bestGeneration[0]):
                        ...
                        bestGeneration[1] = "Gen: " + \
                            str(g) + " Var: " + str(v) + \
                            " Score: " + str(curScore)
                        bestGeneration[2] = wv[v]
                        bestGeneration[3] = bv[v]
                        print("Gen: " + str(g) + " Var: " + str(v) +
                              " Score: " + str(curScore) + " New best score!")
                    else:
